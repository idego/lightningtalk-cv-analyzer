...
Every analysis SHALL be persisted in the API SQLite volume with the authenticated Better Auth user id as its stable `owner_user_id`. The browser MUST NOT receive an analysis-owner capability token; the authenticated Next.js proxy forwards the server-derived owner id to the private API. `GET /analyses` lists the caller's analyses, `GET /analyses/{id}` returns a stored report with capability flags attached, `GET /analyses/{id}/diagnostics` returns usage and cost diagnostics, `GET /analyses/{id}/usage` returns the owner-scoped AI cost estimate, `GET /analyses/{id}/document` streams the stored source document, `POST /analyses/{id}/share` creates a share link, `POST /analyze/cancel` records a cancel for the caller's client request id in the database (`analysis_cancel_requests`, one-hour TTL, purged by retention maintenance) so any API process honors it before the run starts or before the report is persisted, marking `analysis_runs.cancel_requested_at`, and `DELETE /analyses/{id}` and `DELETE /analyses` remove records. `GET /internal/usage/summary` is reachable only with the internal API secret. Records the caller does not own SHALL be indistinguishable from missing ones. Every report the API returns to the web layer (`POST /analyze`, `GET /analyses/{id}`, `GET /shared/analyses/{id}`) SHALL be a browser projection of the stored payload: `versions`, `usage`, `limitations`, `source.sha256`, `source.identity`, per-pass model/usage/latency/attempt/failure details (only `status` and `section_status` remain), and the review internals `rejected`, `conflicts`, `merge_projections`, `relation_corrections` are removed; the stored report and diagnostics keep them. Analysis data is transient and recruiter-owned. Deletion (`DELETE /analyses/{id}`, `DELETE /analyses`) and retention purge remove recruiter-owned analysis records (reports, runs, source documents, per-analysis research and cache-audit rows, audit logs), but SHALL leave long-lived platform records (the AI usage ledger and contextual feedback data) intact.
...
`GET /settings/retention` SHALL expose the deployment-wide retention window in days, initially `CV_VALIDATOR_RETENTION_DAYS` (default 10, 1-3650; the API refuses to start outside that range). A value written through `PUT /settings/retention` is persisted in the database and takes precedence over the environment variable on later restarts. The web proxy adds a `canManage` flag derived from the caller's feedback role. The authenticated web boundary SHALL allow `PUT /settings/retention` only to an active feedback `owner`; the private API SHALL additionally require the web-to-API internal admin secret for that write. The Settings UI SHALL explicitly confirm that the change is deployment-wide and applies to analyses stored from then on. Out-of-range values are rejected with 422 `retention_days_out_of_range`. Every report and analysis run SHALL store an `expires_at` deadline computed from the retention window in effect when the row is written; `GET /analyses` items carry it. Expiration is decided only by that stored deadline (the run's deadline applies only to report-less runs), so changing the retention setting never shortens or extends analyses already stored. Rows written before the deadline existed are backfilled from `created_at` plus the current window on startup. Purge runs on API startup, daily at the configured UTC time (`CV_VALIDATOR_MAINTENANCE_TIME_UTC`, `HH:MM`, default 18:00) in a background loop, and on lifecycle/list writes. Each scheduled purge is followed by a SQLite `VACUUM`, and every store connection sets `PRAGMA secure_delete`. A failed startup or scheduled purge SHALL flip the `retention_purge` capability in `GET /health` to not ready with reason `retention_purge_failed`, which makes the top-level `ready` false until a later analysis purge succeeds; `GET /operations/status` SHALL expose the loop state under `retention.maintenance` (`ready`, `startup_purge_failed`, `last_purge_failed`, `last_purge_at`, `last_vacuum_failed`, `last_vacuum_at`, `run_at`, `next_run_at`). The same loop purges expired Profile Builder profiles. Purging expired analyses SHALL NOT delete associated feedback data or AI usage ledger rows.

#### Scenario: Scheduled purge fails
- **WHEN** the daily purge raises a persistence error
- **THEN** `GET /health` reports `retention_purge` not ready and `ready` false until a subsequent purge succeeds

#### Scenario: Retention lowered
- **WHEN** retention is lowered after analyses were stored
- **THEN** those analyses keep their original `expires_at` and are purged only when it passes, while analyses stored afterwards receive the shorter deadline
