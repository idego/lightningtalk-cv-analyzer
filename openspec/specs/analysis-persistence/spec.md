...
`GET /settings/retention` SHALL expose the deployment-wide retention window in days, defaulting to `CV_VALIDATOR_RETENTION_DAYS` (10). The authenticated web boundary SHALL allow `PUT /settings/retention` only to an active feedback `owner`; the private API SHALL additionally require the web-to-API internal admin secret for that write. The Settings UI SHALL explicitly confirm that the change is deployment-wide and applies to analyses stored from then on. Out-of-range values are rejected with 422 `retention_days_out_of_range`. Every report and analysis run SHALL store an `expires_at` deadline computed from the retention window in effect when the row is written; `GET /analyses` items carry it. Expiration is decided only by that stored deadline (the run's deadline applies only to report-less runs), so changing the retention setting never shortens or extends analyses already stored. Rows written before the deadline existed are backfilled from `created_at` plus the current window on startup. Purge runs on API startup, daily at the configured UTC time (`CV_VALIDATOR_MAINTENANCE_TIME_UTC`, `HH:MM`, default 03:00) in a background loop, and on lifecycle/list writes. Purging expired analyses SHALL NOT delete associated feedback data or AI usage ledger rows.
