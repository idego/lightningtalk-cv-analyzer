...
A signed-in user who can open an analysis SHALL be able to rate any target `helpful` or `not_helpful`, optionally choose one reason (`inaccurate`, `missing_context`, `misleading_importance`, `duplicate`, `unclear`, `stale_research`, `wrong_source`, `operation_failed`, `other`), and optionally add a comment. The response SHALL retain the author's email and a snapshot of the displayed section (`context_label` up to 200 characters, `context_text` up to 12000 characters) so maintainers can review it in context. A comment SHALL be 12 to 180 characters after whitespace normalization and MUST NOT contain URLs, email addresses, or phone numbers. Writes use `PUT /analyses/{analysis_id}/feedback/{target_id}` through the web proxy, which caps a request body at 16 KiB. One actor may write at most 30 times per minute per analysis.
