...
startup and then every 24 hours while running; each scheduled purge is
followed by a SQLite `VACUUM` so freed pages leave the database file. All connections set
