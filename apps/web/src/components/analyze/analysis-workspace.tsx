...
import { useIsNarrowWorkspace } from "@/hooks/use-mobile";
...
export function AnalysisWorkspace({
  ...
  const { t } = useCopy();
  const isNarrowWorkspace = useIsNarrowWorkspace();
  const hostRef = useRef<HTMLDivElement>(null);
  const [activeIndex, setActiveIndex] = useState(0);
  const [previewMode, setPreviewMode] = useState<"auto" | "shown" | "hidden">("auto");
  ...
  const previewVisible = previewMode === "shown" || (previewMode === "auto" && !compact && !isNarrowWorkspace);
  ...
  const columns = !previewVisible || !hasOriginalFiles || isNarrowWorkspace
    ? isNarrowWorkspace ? "minmax(0, 1fr)" : "minmax(0, 1fr) 0px minmax(0, 0fr)"
      ...
      {active.file ? <>
        ...
        >
          <div className={`min-h-0 min-w-0 ${previewVisible ? "overflow-visible" : "overflow-hidden"}`}>
            <DocumentPreview source={active.file} onHide={() => setPreviewMode("hidden")} />
          </div>
        </div>
      </> : null}
    </div>
