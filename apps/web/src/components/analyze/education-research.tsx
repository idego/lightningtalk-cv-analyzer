...
export function EducationResearchPanel({ report }: { report: AnalysisReport }) {
  ...
  return <section className="space-y-3 rounded-md border border-emerald-500/30 p-3">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h3 className="font-medium">Education &amp; Certification Research</h3><p className="text-xs text-muted-foreground">Separate public-web review; it does not change the score or band.</p></div>
      <Button type="button" variant="outline" onClick={startResearch} disabled={!enabled || busy || completed}>
        {busy ? "Researching…" : completed ? "Completed" : "Start education research"}
      </Button>
    </div>
