      ...
      <HoverDisclosure
        ...
        action={
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {feedbackTarget(feedbackManifest, "linkedin_research_result", "linkedin_research", String(profileIndex)) ? <FeedbackControl analysisId={analysisId} target={feedbackTarget(feedbackManifest, "linkedin_research_result", "linkedin_research", String(profileIndex))!} /> : null}
          </div>
        }
...
export function LinkedInResearchPanel({
  ...
  >
    ...
    <div className="space-y-2">
      {sortByResearchConfidence(visibleDiscovery?.possible_profiles ?? []).map((profile) => {
        const profileIndex = visibleDiscovery?.possible_profiles.indexOf(profile) ?? 0;
        return (
        <LinkedInProfileCard
