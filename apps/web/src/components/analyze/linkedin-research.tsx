...
export function LinkedInResearchPanel({ report }: { report: AnalysisReport }) {
  ...
  return <section className="space-y-3 rounded-md border border-sky-500/30 p-3">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-medium">LinkedIn: discovery → confirmation → comparison</h3><p className="text-xs text-muted-foreground">Possible public profiles only. This workflow does not establish identity or change the score or band.</p></div><Button type="button" variant="outline" onClick={discover} disabled={!enabled || discoveryBusy || discoveryCompleted}>{discoveryBusy ? "Discovering…" : discoveryCompleted ? "Discovery completed" : "Start discovery"}</Button></div>
    ...
    {visibleDiscovery?.possible_profiles.map((profile) => <article key={profile.profile_url} className="space-y-2 rounded-md bg-muted/20 p-3 text-sm"><div className="flex flex-wrap items-center gap-2"><a href={profile.profile_url} target="_blank" rel="noreferrer" className="underline">Possible profile</a><Badge variant="outline">confidence: {profile.confidence}</Badge><Badge variant="outline">photo visible: {profile.photo_visible}</Badge><Badge variant="outline">connections: {profile.connection_count.display ?? "unknown"}</Badge>{profile.connection_completeness_flag ? <Badge variant="outline">completeness review</Badge> : null}</div><p className="text-xs text-muted-foreground">{profile.uncertainty}</p>{profile.match_evidence.map((item, i)=><div key={`${item.field}-${i}`} className="text-xs"><p>Match evidence · {item.field}: {item.cv_value} ↔ {item.profile_value}</p>{item.source_urls.map(url=><a key={url} href={url} target="_blank" rel="noreferrer" className="mr-2 underline">Source</a>)}</div>)}{profile.conflicts.map((item, i)=><div key={`${item.field}-${i}`} className="text-xs text-amber-700"><p>Conflict for review · {item.summary}</p>{item.source_urls.map(url=><a key={url} href={url} target="_blank" rel="noreferrer" className="mr-2 underline">Source</a>)}</div>)}{profile.photo_source_url ? <a href={profile.photo_source_url} target="_blank" rel="noreferrer" className="text-xs underline">Photo visibility source</a> : null}{profile.connection_count.source_url ? <a href={profile.connection_count.source_url} target="_blank" rel="noreferrer" className="ml-2 text-xs underline">Connection count source</a> : null}<Button type="button" size="sm" variant="outline" disabled={Boolean(confirmed)} onClick={()=>confirm(profile.profile_url)}>{confirmed === profile.profile_url ? "Confirmed for comparison" : "Confirm this URL for comparison"}</Button></article>)}
