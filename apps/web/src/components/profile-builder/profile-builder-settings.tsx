  ...
  return <div className="space-y-6" id="profile-builder-settings">
    <section className="rounded-xl border bg-card p-5">
      ...
      <div className="mt-3 divide-y">
        ...
        <div className="py-3"><p className="mb-1 text-sm font-medium">Default anonymization</p><div className="grid gap-x-6 sm:grid-cols-2">{([['First name','hide_first_name'],['Last name','hide_last_name'],['Email','hide_email'],['Phone','hide_phone'],['Location','hide_location'],['LinkedIn','hide_linkedin'],['GitHub','hide_github'],['Portfolio','hide_portfolio'],['Other links','hide_other_links']] as const).map(([label,key]) => toggle(`profile-default-${key}`, `Hide ${label}`, profileBuilderPreferences.anonymization[key], checked => setProfileBuilderPreferences((current) => ({ ...current, anonymization: { ...current.anonymization, [key]: checked } }))))}</div><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1 text-sm"><span>Employer names</span><select value={profileBuilderPreferences.anonymization.employer_mode} onChange={(event) => setProfileBuilderPreferences((current) => ({ ...current, anonymization: { ...current.anonymization, employer_mode: event.target.value as AnonymizationPolicy["employer_mode"] } }))} className="h-10 w-full rounded-md border bg-background px-3"><option value="show">Show</option><option value="hide">Hide</option><option value="genericize">Company category</option></select></label><label className="space-y-1 text-sm"><span>Institution names</span><select value={profileBuilderPreferences.anonymization.institution_mode} onChange={(event) => setProfileBuilderPreferences((current) => ({ ...current, anonymization: { ...current.anonymization, institution_mode: event.target.value as AnonymizationPolicy["institution_mode"] } }))} className="h-10 w-full rounded-md border bg-background px-3"><option value="show">Show</option><option value="hide">Hide</option></select></label></div></div>
