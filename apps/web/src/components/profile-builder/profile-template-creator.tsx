...
export function ProfileTemplateCreator({ templateId, returnProfileId }: { templateId: string | null; returnProfileId?: string | null }) {
  ...
  const [workspacePane, setWorkspacePane] = useState<"blocks" | "preview" | "properties">("preview");
  ...
  async function saveTemplate() {
    ...
    if (payload.id === "new") payload.id = creatorTemplateId();
  ...
  if (loading) {
    return <div role="status" className="flex h-full min-h-0 items-center justify-center gap-2 text-sm text-muted-foreground"><LoaderCircle className="size-5 animate-spin" />Loading template…</div>;
  }

  if (loadFailed) return <div role="alert" className="mx-auto max-w-lg space-y-4 rounded-xl border bg-card p-5"><h1 className="font-semibold">Template unavailable</h1><p className="text-sm text-muted-foreground">It may have been deleted or shared access may have changed.</p><div className="flex gap-2"><Button variant="outline" onClick={leaveCreator}>Back</Button><Button onClick={() => setLoadAttempt((value) => value + 1)}>Retry</Button></div></div>;
  ...
  return (
    <div className="profile-creator-shell flex h-full min-h-0 w-full flex-col gap-3 overflow-hidden" data-creator-pane={workspacePane}>
      ...
      {error ? <p role="alert" className="shrink-0 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">{error}</p> : null}
      <div className="profile-creator-switch flex shrink-0 gap-1 rounded-lg border bg-card p-1" role="group" aria-label="Template editor view">
      ...
      <div className="profile-creator-columns grid min-h-0 flex-1 gap-3">
              ...
              {template.sections.map((section, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => { setSelectedSectionId(section.id); setInspectorTab("block"); setWorkspacePane("properties"); }}
        ...
        <div className="profile-creator-preview min-h-0 overflow-hidden rounded-xl border bg-muted/10 p-2">
          <ProfileDocumentPreview
            ...
            onSectionSelect={(sectionId) => { setSelectedSectionId(sectionId); setInspectorTab("block"); setWorkspacePane("properties"); }}
            logoEditable
            onLogoSelect={() => { setInspectorTab("logo"); setWorkspacePane("properties"); }}
