  ...
  return {
    ...
    personal: {
      ...
      links: {
        ...
        other: policy.hide_other_links ? [] : profile.personal.links.other,
      },
    },
...
/** Recruiter-readable comparison text; internal record IDs never enter the review UI. */
export function formatProfessionalSection(section: ProfessionalSectionName, value: ProfessionalProposal[ProfessionalSectionName]): string {
  if (value == null || value === "") return "Not provided";
  if (typeof value === "string") return value;
  const join = (parts: Array<string | null | undefined>) => parts.filter(Boolean).join(" · ");
  const dates = (entry: { start_date: string | null; end_date: string | null; current?: boolean }) =>
    entry.start_date && (entry.end_date || entry.current) ? `${entry.start_date} – ${entry.current ? "Present" : entry.end_date}` : entry.start_date ?? entry.end_date;
  const bullets = (items: string[]) => items.map((item) => `• ${item}`).join("\n");
  switch (section) {
    case "skills": case "technologies": return bullets(value as string[]) || "Not provided";
    case "experience": return (value as CandidateProfile["experience"]).map((entry) => [
      join([entry.role, entry.company]), join([dates(entry), entry.location, entry.project]),
      bullets(entry.responsibilities), entry.achievements.length ? `Highlights:\n${bullets(entry.achievements)}` : "",
      entry.technologies.length ? `Technologies: ${entry.technologies.join(", ")}` : "",
    ].filter(Boolean).join("\n")).join("\n\n") || "Not provided";
    case "education": return (value as CandidateProfile["education"]).map((entry) => [join([entry.institution, entry.degree, entry.field]), join([dates(entry), entry.location]), entry.description].filter(Boolean).join("\n")).join("\n\n") || "Not provided";
    case "languages": return (value as CandidateProfile["languages"]).map((entry) => join([entry.language, entry.level])).join("\n") || "Not provided";
    case "certifications": return (value as CandidateProfile["certifications"]).map((entry) => join([entry.name, entry.issuer, entry.date, entry.url])).join("\n") || "Not provided";
    case "additional_sections": return (value as CandidateProfile["additional_sections"]).map((entry) => `${entry.title}\n${bullets(entry.items)}`).join("\n\n") || "Not provided";
