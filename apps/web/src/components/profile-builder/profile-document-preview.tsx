...
export function ProfileDocumentPreview({
  ...
  }, [fillHeight, pageWidth, pageHeight]);
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      // Focus/drag outlines can extend a few pixels beyond a CSS column.
      const count = Math.max(1, Math.round(flow.scrollWidth / pageWidth));
  ...
  function draggableSection(section: ProfileTemplateSection) {
    return (
      <div
        ...
        role={sectionsEditable ? "button" : undefined}
        tabIndex={sectionsEditable ? 0 : undefined}
        aria-label={sectionsEditable ? `Edit ${section.title} block` : undefined}
        onKeyDown={(event) => { if (sectionsEditable && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onSectionSelect?.(section.id); } }}
        ...
        <div ref={previewViewportRef} className="absolute inset-x-[9.07%] bottom-[5.56%] top-[5.56%] overflow-hidden">
          <div
            ...
            style={{
              ...
              fontFamily: `${template.typography.font_family}, Arial, sans-serif`,
              lineHeight: 1.15,
              overflowWrap: "anywhere",
              fontSize: `${pointsToPixels(template.typography.body_size)}px`,
            }}
          >
            <div className="mb-5 flex break-inside-avoid items-center justify-between gap-4 border-b-2 pb-4" style={{ borderColor: template.branding.accent_hex }}>
              <div>
