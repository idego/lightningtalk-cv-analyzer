...
import { isAnalysisReport, isReportModuleCategory, ReportModule } from "@/components/analyze/report-module";
...
type InboxItem = {
  ...
  analysis_available?: boolean; context_expired?: boolean;
};
              ...
              <div className="border-t px-4 py-4 sm:px-5">
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)]">
                  <div className="min-w-0 space-y-3">
                    ...
                    {moduleReport && moduleCategory ? (
                      <details className="group border-t pt-3">
                        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-lg py-1 text-sm font-medium outline-none hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring">
                          <span className="flex items-center gap-2"><FileText className="size-4 text-muted-foreground" />{t("showReportModule")}</span><ChevronDown className="size-4 text-muted-foreground transition-transform group-open:rotate-180" />
                        </summary>
                        <div className="mt-3 space-y-2">
                          {!sameLabel(item.context_label, context.section) && !sameLabel(item.context_label, context.subject) ? <p className="text-xs font-semibold text-muted-foreground">{item.context_label}</p> : null}
                    ...
                    ) : moduleCategory && (item.analysis_available === false || item.context_expired) ? (
                      <div className="border-t pt-3">
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">{t(item.analysis_available === false ? "reportModuleAnalysisDeleted" : "reportModuleContextExpired")}</p>
                      </div>
                    ) : null}
                  ...
                  <div className="border-t pt-4 lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0">
                    ...
                    <textarea id={`note-${key}`} className={`mt-2 w-full resize-y rounded-lg border bg-background px-3 py-2 text-sm leading-relaxed outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 ${item.comment || item.context_text || moduleReport || item.failure ? "min-h-24" : "min-h-16"}`} maxLength={500} placeholder={t("teamNotePlaceholder")} value={notes[key] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [key]: event.target.value }))} />
