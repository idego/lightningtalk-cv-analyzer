...
const statusLabels: Record<string, string> = { new: "New", reviewing: "Reviewing", planned: "Planned", resolved: "Resolved", wont_fix: "Won't fix" };
...
export function FeedbackInbox({ owner }: { owner: boolean }) {
  ...
  useEffect(() => {
    ...
    const message = "Masz niezapisaną notatkę do feedbacku. Czy na pewno chcesz opuścić stronę?";
  ...
  async function updateTriage(item: InboxItem, nextStatus: string, note: string) {
    ...
    else setErrors((current) => ({ ...current, [key]: "Nie udało się zapisać zmian." }));
    setBusy(null);
  }
  ...
  async function remove(item: InboxItem) {
    ...
    else setErrors((current) => ({ ...current, [key]: "Nie udało się usunąć feedbacku." }));
    setBusy(null);
  }
      ...
      <div className="flex items-start justify-between gap-4">
        <details className="group relative w-fit rounded-lg border bg-card">
          <summary className="flex min-w-40 cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 text-sm font-medium transition-colors hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset">
            <span className="flex min-w-0 items-center gap-2"><SlidersHorizontal className="size-4 text-muted-foreground" /><span>Filtry</span>{status ? <span className="truncate text-xs font-normal text-muted-foreground">· {statusLabels[status]}</span> : null}</span>
            <ChevronDown className="size-4 text-muted-foreground transition-transform group-open:rotate-180" />
          </summary>
          <div className="absolute left-0 top-full z-30 mt-2 flex w-[min(32rem,calc(100vw-2rem))] flex-wrap gap-2 rounded-xl border bg-popover p-3 text-popover-foreground shadow-md" aria-label="Filtry statusu">
            {["", ...statuses].map((value) => (
              <Button key={value} variant={status === value ? "secondary" : "outline"} size="sm" className="rounded-full" onClick={() => setStatus(value)} aria-pressed={status === value}>
                {value ? statusLabels[value] : "Wszystkie"}<span className="tabular-nums text-muted-foreground">{value ? data?.counts[value] ?? 0 : Object.values(data?.counts ?? {}).reduce((sum, count) => sum + count, 0)}</span>
              </Button>
            ))}
          </div>
        </details>
        {owner ? <Button variant="outline" nativeButton={false} render={<Link href="/feedback/access">Zarządzaj dostępem</Link>} /> : null}
      </div>

      <div className="space-y-3">
        {!data ? <div className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground"><LoaderCircle className="size-4 animate-spin" />Ładowanie feedbacku…</div> : null}
        {data?.items.map((item) => {
          const context = feedbackContext(item);
          const key = itemKey(item);
          const awaitingConfirmation = confirmDelete === key;
          const isBusy = busy === key;
          const date = item.updated_at ? new Date(item.updated_at).toLocaleString("pl-PL", { dateStyle: "medium", timeStyle: "short" }) : null;
          return (
              ...
              <header className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-5">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                    ...
                    {item.rating ? <span className="inline-flex items-center gap-1 rounded-md bg-muted px-1.5 py-0.5 text-xs text-muted-foreground">{item.rating === "helpful" ? <ThumbsUp className="size-3" /> : <ThumbsDown className="size-3" />}{item.rating === "helpful" ? "Pomocny" : "Do poprawy"}</span> : null}
                ...
                <div className="flex items-center gap-1.5">
                  <DropdownMenu>
                    <DropdownMenuTrigger disabled={isBusy} render={<Button variant="outline" className="min-w-32 justify-between" />}>
                      <span className="flex items-center gap-2"><span className={`size-2 rounded-full ${statusDot(item.triage_status)}`} aria-hidden="true" />{statusLabels[item.triage_status]}</span><ChevronDown className="size-4 text-muted-foreground" />
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" side="bottom" sideOffset={6} className="min-w-40">
                      {statuses.map((value) => (
                        <DropdownMenuItem key={value} className="py-2" onClick={() => void updateTriage(item, value, notes[key] ?? "")}>
                          <span className={`size-2 rounded-full ${statusDot(value)}`} aria-hidden="true" /><span className="flex-1">{statusLabels[value]}</span>{item.triage_status === value ? <span className="text-xs text-muted-foreground">Aktualny</span> : null}
                        </DropdownMenuItem>
                      ))}
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <div className="relative">
                    <Button
                      ...
                      disabled={isBusy}
                      aria-label={awaitingConfirmation ? "Kliknij ponownie, aby usunąć feedback" : "Usuń feedback"}
                    ...
                    {awaitingConfirmation ? <span role="status" className="absolute right-0 top-full z-20 mt-2 whitespace-nowrap rounded-md bg-foreground px-2 py-1 text-xs text-background shadow-md">Kliknij ponownie, aby potwierdzić</span> : null}
                  </div>
                </div>
              </header>
              ...
              <div className="border-t px-4 py-4 sm:px-5">
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)]">
                  <div className="min-w-0 space-y-3">
                    <div><p className="text-xs font-medium text-muted-foreground">Komentarz od: {item.actor_email || "autor nieznany"}</p><p className="mt-1.5 text-sm leading-relaxed">{item.comment || "Bez komentarza"}</p></div>
                    {item.context_text ? (
                      <details className="group border-t pt-3">
                        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-lg py-1 text-sm font-medium outline-none hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring">
                          <span className="flex items-center gap-2"><FileText className="size-4 text-muted-foreground" />Pokaż fragment CV</span><ChevronDown className="size-4 text-muted-foreground transition-transform group-open:rotate-180" />
                        </summary>
                    ...
                    {item.failure ? <details className="border-t pt-3"><summary className="cursor-pointer text-sm font-medium">Szczegóły błędu</summary><pre className="mt-3 max-h-64 overflow-auto rounded-lg bg-muted p-3 text-xs">{JSON.stringify(item.failure, null, 2)}</pre></details> : null}
                  </div>

                  <div className="border-t pt-4 lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0">
                    <label className="flex items-center gap-2 text-xs font-medium text-muted-foreground" htmlFor={`note-${key}`}><MessageSquareText className="size-4" />Notatka zespołu</label>
                    <textarea id={`note-${key}`} className="mt-2 min-h-24 w-full resize-y rounded-lg border bg-background px-3 py-2 text-sm leading-relaxed outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50" maxLength={500} placeholder="Dodaj kontekst, decyzję albo kolejny krok…" value={notes[key] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [key]: event.target.value }))} />
                    <div className="mt-2 flex min-h-8 flex-wrap items-center justify-between gap-2"><span className="text-xs text-destructive" role="alert">{errors[key]}</span><Button size="sm" disabled={isBusy || (notes[key] ?? "").trim() === (item.triage_note ?? "")} onClick={() => void updateTriage(item, item.triage_status, notes[key] ?? "")}>{isBusy ? <LoaderCircle className="animate-spin" /> : null}Zapisz notatkę</Button></div>
                  </div>
                </div>
              </div>
            </article>
        ...
        {data && !data.items.length ? <p className="py-16 text-center text-sm text-muted-foreground">Brak feedbacku dla wybranego filtra.</p> : null}
      </div>
    </section>
  );
}
