...
def _target_candidates(payload: dict[str, Any]):
    ...
    for key, kind in (("company_research", TargetKind.COMPANY_RESEARCH_RESULT), ("education_research", TargetKind.EDUCATION_RESEARCH_RESULT), ("linkedin_research", TargetKind.LINKEDIN_RESEARCH_RESULT)):
        result = payload.get(key)
        if isinstance(result, dict) and result.get("status") == "completed":
            values = result.get("organizations") or result.get("institutions") or result.get("profiles") or []
...
def _versions(payload: dict[str, Any]) -> dict[str, str]:
    values = {"ruleset": str(payload.get("ruleset_version") or "unknown")[:80]}
    ai = payload.get("ai_analysis")
