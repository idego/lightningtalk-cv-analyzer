...
_HEADINGS = {
    "employment": {"experience", "employment", "work experience", "doswiadczenie", "doswiadczenie zawodowe", "zatrudnienie"},
    "education": {"education", "academic background", "edukacja", "wyksztalcenie"},
    "other": {"contact", "personal details", "certifications", "projects", "publications", "awards", "kontakt", "dane osobowe", "certyfikaty", "projekty", "publikacje", "nagrody"},
}
...
def _timeline(document: RedactedDocument, snapshot: str, cfg: StructuralAuditConfig) -> TimelineAudit:
    entries: list[TimelineEntry] = []
    category = "unknown"
    for page in document.pages:
        for line in page.lines:
            normalized = _normalize(line.text)
            heading = next((kind for kind, values in _HEADINGS.items() if normalized in values), None)
            if heading:
                category = heading if heading != "other" else "unknown"
                continue
