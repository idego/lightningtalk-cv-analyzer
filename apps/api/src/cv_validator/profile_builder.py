...
class AnonymizationPolicy(_StrictModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def protect_legacy_hidden_links(cls, value):
        if isinstance(value, dict) and "hide_other_links" not in value:
            return {**value, "hide_other_links": all(value.get(key) is True for key in ("hide_linkedin", "hide_github", "hide_portfolio"))}
        return value
